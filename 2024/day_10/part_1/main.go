...
func main() {
	...
	for i := 0; i < m; i++ {
		for j := 0; j < n; j++ {
			if mp[i][j] == 0 {
				ans += calculateScore(i, j, 0, mp, utils.Make2DArr[bool](m, n))
			}
		}
	}
...
func calculateScore(i, j, h int, mp [][]int, done [][]bool) int {
	m, n := len(mp), len(mp[0])

	if mp[i][j] == 9 && !done[i][j] {
		done[i][j] = true
		return 1
	}
	...
	for _, d := range utils.DiffAdj {
		if (utils.Vec{X: i, Y: j}).Add(d).WithinBounds(m, n) && mp[i+d.X][j+d.Y] == h+1 {
			score += calculateScore(i+d.X, j+d.Y, h+1, mp, done)
		}
	}
